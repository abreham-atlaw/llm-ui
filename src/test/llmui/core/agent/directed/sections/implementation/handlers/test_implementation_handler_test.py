...
from llmui.core.agent.directed.sections.implementation.handlers.test_implementation_handler import \
	TestImplementationHandlerArgs, TestImplementationHandler
...
class TestImplementationHandlerTest(unittest.TestCase):
	...
	def test_functionality(self):
		...
		handler = TestImplementationHandler(LLMProviders.provide_console_llm())

		while handler.internal_state.stage != ImplementationStage.done:
			action = handler._handle(
				environment.state,
				TestImplementationHandlerArgs(
					project_info=ProjectInfo(
						task="Phase 1: Implement Services Page\n1. Create the HTML structure for the Services Page, ensuring that each service is described within a card with a pill-like shape and high border radius.\n2. Apply styling to the services cards, giving them a curved aesthetic with rounded corners to maintain the overall design theme.\n3. Implement a carousel for case studies on the Services Page, ensuring that it has curved edges to provide a smooth visual flow and enhance the user experience.",
						tech_stack=["Vue3", "Tailwind", "Typescript", "Ubuntu"],
						ignored_files=[
					...
					files=[
						"orbit-website/src/components/Services.vue",
			...
			handler.save("/home/abreham/Projects/TeamProjects/LLM-UI/temp/test.json")

		self.assertTrue(handler.internal_state.stage == ImplementationStage.done)
